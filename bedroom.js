...
function preload() {
    img = loadImage("bedroom.jpg"); //image loaded
}
...
function draw() {
    ...
    if (status != "") { //if the status is true
        for (i = 0; i < objects.length; i++) { //loop
            ...
            noStroke(); 
            text(name + " " + percent + "%", x+20, y - 130); //text label shown

            noFill();

            strokeWeight(2); //line width
            stroke("white"); //line colours
            rect(x-100, y - 150, width, height); //rectangle drawn

            document.getElementById("status").innerHTML = "There are 2 big objects in the image out of which CocoSsd Model has detected 2."; //html status changed
        }
    }
}