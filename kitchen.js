...
function preload() {
    img = loadImage("kitchen.jpeg"); //image loaded
}
...
function draw() {
    ...
    if (status != "") { //if the status is true
        for (i = 0; i < objects.length; i++) { //loop
            ...
            document.getElementById("status").innerHTML = "There are 4 big objects in the image out of which CocoSsd Model has detected _."; //html status changed
        }
    }
}