...
function preload() {
    img = loadImage("bathroom.webp"); //image loaded
}
...
function draw() {
    ...
    if (status != "") { //if the status is true
        for (i = 0; i < objects.length; i++) { //loop
            ...
            rect(x, y - 150, width, height); //rectangle drawn

            document.getElementById("status").innerHTML = "There are 3 big objects in the image out of which CocoSsd Model has detected 1."; //html status changed
        }
    }
}